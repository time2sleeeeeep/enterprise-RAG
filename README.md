...
│                                                                              │
│ ▸ 文档摄入管线 (/documents)                                                  │
│   加载(PDF/MD/DOCX) → 清洗 → 分块(递归512/64 + 语义)                         │
│   → 向量化 [BGE-M3] → 写入 [Milvus] [MySQL]                                  │
│                                                                              │
│ ▸ 核心 RAG 管线 (/chat 触发, /eval 复用检索部分)                             │
│   缓存检查 [Redis] → 查询向量化 [BGE-M3] → 混合检索 [Milvus]                 │
│   → RRF 融合 (k=60) → 重排序 [BGE-Reranker]                                  │
│   → 历史注入 [MySQL] → LLM 生成 [DeepSeek]                                   │
│   → 写入缓存 [Redis] + 保存历史 [MySQL]                                      │
│                                                                              │
│ ▸ 评估框架 (/eval)                                                           │
│   复用核心 RAG 管线(检索+生成) → LLM 裁判打分 [DeepSeek]                     │
│   → 检索指标计算 (Prec@K, Rec@K, MRR, NDCG@K)                                │
│   → 统计评估 → 报告生成 (HTML/MD)                                            │
│                                                                              │
│ ▸ 健康检查 (/health)                                                         │
│   并发探活: Milvus(list_collections) + MySQL(SELECT 1) + Redis(PING)         │
│   各 2s 超时, 任一不可用 → 503                                               │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
       │                  │                  │                │
       ▼                  ▼                  ▼                ▼
      ┌────────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────────┐
      │   Redis    │    │    Milvus    │    │    MySQL     │    │    DeepSeek      │
      │  语义缓存   │    │  向量数据库   │    │ 元数据/历史   │    │   外部 LLM API   │
      └────────────┘    └──────────────┘    └──────────────┘    └──────────────────┘

  注: BGE-M3 (~2GB) 和 BGE-Reranker (~1.5GB) 运行于 FastAPI 进程内,
  本地 GPU 单例加载, 非独立服务。管线步骤通过 [...] 标注其所依赖的组件。
```
